...
const TaskBoard = ({ defaultTaskId = null }) => {
    ...
    const fetchThreads = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (!defaultTaskId && loadedThreads.length > 0) {
            setSelectedId(loadedThreads[0].id);
        ...
        notify(`Failed to fetch workflow threads: ${err.message}`, 'error');
      }
    };
    fetchThreads();
  }, [defaultTaskId]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error(err);
        notify(`Failed to load workflow data: ${err.message}`, 'error');
        setWorkflowState(null);
        setAnalyticsData([]);
        setEvents([]);
      }
    ...
    ws = new WebSocket(`${wsUrl}/workflows/${selectedId}/events?api_key=${API_KEY}`);
    ws.onmessage = (event) => {
      try {
        ...
        setEvents(prev => {
          ...
          if (msg.performative === 'failure') {
            notify(msg.payload.natural_language, 'error', 10000);
          }
  ...
  const submitHumanResponse = async (agentId) => {
    ...
    try {
      await fetch(`${API_URL}/workflows/${selectedId}/input`, {
        method: 'POST',
        headers: {
      ...
      <div className={`col-span-12 lg:col-span-6 xl:col-span-7 h-full bg-[#0E0E0E] rounded-[24px] border border-white/[0.07] overflow-hidden shadow-2xl ${mobilePanel === 'main' ? 'flex' : 'hidden lg:flex'} flex-col`}>
        ...
        {centerTab === 'graph' && (
          <div className="flex-grow relative overflow-hidden">
            <GraphView tasks={workflowState?.tasks} manifest={workflowState?.manifest} analytics={analyticsData} />
          </div>
        )}
